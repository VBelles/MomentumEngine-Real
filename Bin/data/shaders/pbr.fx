...
) : SV_Target
{
	...
	float3 env = getEnvironment(reflected_dir, mipIndex);
	...
	float3 irradiance_mipmaps = getEnvironment(N, 6);
	float3 irradiance_texture = getIrradiance(N);
...
float4 shade(
	...
	float  att = (1. - smoothstep(0.60, 0.98, distance_to_light / light_radius));
