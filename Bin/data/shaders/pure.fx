...
)
{
  ...
  //world_pos.xyz += oNormal * 0.2 * sin( 1 * global_world_time + world_pos.x * 0.0 + world_pos.z * 0.0);
...
)  : SV_Target0
{
  ...
  float2 noiseF = noise0.xy + noise1.xy * 0.5 + noise2.xy * .25;
	...
	float2 pos_camera_unit_space = float2(
		(1 + pos_homo_space.x) * 0.5 + 0.001 * (1 - sin( 11 * global_world_time + iWorldPos.x + iWorldPos.z + iWorldPos.y)), 
		(1 - pos_homo_space.y) * 0.5 + 0.001 * (1 - sin( 18 * global_world_time + iWorldPos.x + iWorldPos.z + iWorldPos.y))
		);
	...
	return lerp(final_color, albedo, 0.6);
}
