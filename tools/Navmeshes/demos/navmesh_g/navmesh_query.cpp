...
void CNavmeshQuery::setStartAndEndPosition(VEC3 start_pos, VEC3 end_pos)
{
	p1.p = start_pos;
	p1.set = true;
	if (data->m_navMesh)
...
void CNavmeshQuery::setStartPosition(VEC3 start_pos)
{
	p1.p = start_pos;
	p1.set = true;
	if (data->m_navMesh)
...
void CNavmeshQuery::setEndPosition(VEC3 end_pos)
{
	p2.p = end_pos;
	p2.set = true;
	if (data->m_navMesh)
...
std::vector< VEC3 > CNavmeshQuery::getPathArray()
{
	std::vector< VEC3 > points;

	for (int i = 0; i < m_nsmoothPath; ++i) {
		points.push_back(VEC3( m_smoothPath[i * 3], m_smoothPath[i * 3 + 1] + 0.1f, m_smoothPath[i * 3 + 2]));
	}
	return points;
}
  ...
  if( tool == ETool::FIND_PATH ) {
    ...
    if( m_nsmoothPath ) {
	  ...
	  for (int i = 0; i < m_nsmoothPath; ++i) {
		 ...
		 // dbg("x:%f y:%f z:%f \n",m_smoothPath[i * 3], m_smoothPath[i * 3 + 1] + 0.1f, m_smoothPath[i * 3 + 2]);
	  }
      dd.end( );
      dd.depthMask( true );
    }
