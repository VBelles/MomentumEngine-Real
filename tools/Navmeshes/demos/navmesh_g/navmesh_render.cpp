...
struct SimpleVertex
{
	VEC3 pos;
	VEC4 color;
	void set(VEC3 new_pos, VEC4 new_color) { pos = new_pos; color = new_color; }
};
//TODO:MARC
void DebugDrawGL::end()
{
	...
	if (primitive == DU_DRAW_POINTS) {
		...
		int num_lines = num_colored_vertices;
		CUSTOMVERTEX vs[2];
		...
		for (int i = 0; i<num_lines; ++i) {
			...
			Render.drawPoints(VEC3(vs[0].x, vs[0].y, vs[0].z), VEC4(1, 0, 0, 1));
		...
		int num_lines = num_colored_vertices / 2;
		CUSTOMVERTEX vs[2];
		...
		for (int i = 0; i<num_lines; ++i) {
			...
			vs[1].assign(v2.x, v2.y, v2.z, v2.color);

			Render.drawLine(VEC3(vs[0].x, vs[0].y, vs[0].z), VEC3(vs[1].x, vs[1].y, vs[1].z), VEC4(1, 0, 0, 1));
			//g_pd3dDevice->DrawPrimitiveUP(D3DPT_TRIANGLELIST, 2, vs, sizeof(CUSTOMVERTEX));
		}
