...
DXGI_FORMAT readFormat(const json& j, const std::string& label) {
    ...
    if (format == "none")           return DXGI_FORMAT_UNKNOWN;
    if (format == "R8G8B8A8_UNORM") return DXGI_FORMAT_R8G8B8A8_UNORM;
    if (format == "R32_TYPELESS")   return DXGI_FORMAT_R32_TYPELESS;
