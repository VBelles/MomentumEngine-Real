...
bool CRenderTechnique::reloadPS() {
    ...
    if (ps_entry_point.empty()) return true;
...
bool CRenderTechnique::create(const std::string& name, json& j) {
    ...
    if (j.count("textures")) {
        ...
        for (auto it = j_textures.begin(); it != j_textures.end(); ++it) {
            TSlot s;
            if (it.key() == "environment")      s.slot = TS_ENVIRONMENT_MAP;
...
void CRenderTechnique::destroy() {
    if (vs) vs->destroy(), vs = nullptr;
    if (ps) ps->destroy(), ps = nullptr;
}
...
void CRenderTechnique::activate() const {
    ...
    // shadows maps
    if (ps) ps->activate();
    else Render.ctx->PSSetShader(nullptr, nullptr, 0);
