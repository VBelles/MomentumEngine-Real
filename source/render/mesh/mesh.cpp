...
bool CRenderMesh::create(const void*        vertex_data,
	...
	if (new_subgroups) {
		...
		if (num_indices > 0) subgroups.push_back({ 0, num_indices, 0, 0 });
		else subgroups.push_back({ 0, num_vertexs, 0, 0 });
	}
...
void CRenderMesh::debugInMenu() {
	...
	if (ib) ImGui::Text("%d Indices", num_indices);
	if (is_dynamic) ImGui::Text("Dynamic");
