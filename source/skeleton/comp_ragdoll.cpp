...
void TCompRagdoll::activateRagdoll() {
  ...
  for (int i = 0; i < ragdoll.num_bones; ++i) {
	  ...
	  CModulePhysics& module_physics = CEngine::get().getPhysics();
	  module_physics.gScene->addActor(*ragdoll_bone.actor);
  }
}
...
void TCompRagdoll::deactivateRagdoll() {
	...
	for (int i = 0; i < ragdoll.num_bones; ++i) {
		...
		CModulePhysics& module_physics = CEngine::get().getPhysics();
		module_physics.gScene->removeActor(*ragdoll_bone.actor);
	}
	updateRagdollFromSkeleton();
...
void TCompRagdoll::renderDebug()  {
  for (int i = 0; i < ragdoll.num_bones; ++i) {
	...
	MAT44 world2 = transform.asMatrix();
	  ...
	  MAT44 physics_world1 = MAT44::CreateScale(ragdoll_bone.core->height, ragdoll_bone.core->radius, ragdoll_bone.core->radius) * world1;
	  MAT44 physics_world2 = MAT44::CreateScale(ragdoll_bone.core->height, ragdoll_bone.core->radius, ragdoll_bone.core->radius) * world2;
