...
void ScriptingCameras::blendInCamera(std::string cameraName, float timeToMixIn, int priority, std::string interpolator) {
	...
	Interpolator::IInterpolator* interp = getInterpolator(interpolator);
	Engine.getCameras().blendInCamera(h_camera, timeToMixIn, prio, interp);
}

Interpolator::IInterpolator* ScriptingCameras::getInterpolator(std::string interpolator) {
	Interpolator::IInterpolator* interp = nullptr;
	if (interpolator == INTERPOLATOR_CUBIC_IN) { interp = new Interpolator::TCubicInInterpolator(); }
