...
namespace VTemplate {
	...
	struct TTemplateAny {
		virtual std::string toString(const VariableCallback& variableCallback) = 0;
	};

	struct TTemplateText : TTemplateAny {
		std::string text;
		TTemplateText(std::string text) : text(text) {}
		std::string toString(const VariableCallback& variableCallback) override { return text; }
	};

	struct TemplateVariable : TTemplateAny {
		std::string name;
		TemplateVariable(std::string name) : name(name) {}
		std::string toString(const VariableCallback& variableCallback) override { return variableCallback(name); }
	};

	struct TTemplate {
		std::vector<TTemplateAny*> texts;
		std::string toString(const VariableCallback& variableCallback) {
			std::string s;
