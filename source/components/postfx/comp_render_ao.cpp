...
void TCompRenderAO::debugInMenu() {
	...
	if (ImGui::DragFloat("ZRange Discard", &zrange_discard, 0.000001f, 0.0f, 0.1f, "%.6f")) {
		zrange_discard_units = zrange_discard * cb_camera.camera_zfar;
	}
	if (ImGui::DragFloat("ZRange Discard (u)", &zrange_discard_units, 0.001f, 0.0f, 0.5f)) {
		zrange_discard = zrange_discard_units / cb_camera.camera_zfar;
	}
