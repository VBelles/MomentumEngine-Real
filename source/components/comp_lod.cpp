...
void TCompLOD::registerMsgs() {
	DECL_MSG(TCompLOD, TMsgEntitiesGroupCreated, onGroupCreated);
}
...
void TCompLOD::debugInMenu() {
	...
	if (ImGui::TreeNode("Children")) {
		for (auto h : handles) h.debugInMenu();
		ImGui::TreePop();
	}
}
...
void TCompLOD::updateAABBFromChildren() {
	...
	for (auto h : handles) {
		...
		if (c_aabb) {
			...
			if (nchildren == 0) aabb = *child_aabb;
			else AABB::CreateMerged(aabb, aabb, *child_aabb);
			++nchildren;
		}
	}
...
void TCompLOD::updateLevel() {
	...
	VEC3 camera_pos = camera_render->getCamera()->getPosition();
...
void TCompLOD::applyLODStatus() {
	...
	for (auto h : handles) h.sendMsg(msg);
}
...
void TCompLOD::update(float delta) {
	...
	if (was_using_lod != using_lod) applyLODStatus();
}
